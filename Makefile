...
shellcheck: ## shellcheck all shell + the rendered user-data template
	...
	$(DKR) -v /tmp/_user_data.rendered.sh:/work/_user_data.rendered.sh $(SHELLCHECK_IMG) \
	  ...
	  deploy/local/verify.sh deploy/local/dev-up.sh deploy/local/deploy-remote.sh deploy/local/snapshot.sh deploy/local/apply-head.sh \
...
.PHONY: apply-theme
apply-theme: ## re-apply ccc-custom-head.html to the running stack's theme (no restart; deploys do this automatically)
	bash deploy/local/apply-head.sh
