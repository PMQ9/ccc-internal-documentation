...
| Date | Change | PR |
|---|---|---|
| 2026-06-05 | Brand theme is now config-as-code — deploys (and `make apply-theme`) auto-apply `ccc-custom-head.html` to BookStack's `app-custom-head` DB setting via `apply-head.sh` (idempotent; file is source of truth). No more manual paste | — |
